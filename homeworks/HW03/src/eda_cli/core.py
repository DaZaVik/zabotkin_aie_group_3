...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    high_cardinality_columns: Dict[str, float] = {}
    if summary.n_rows > 0:
        for c in summary.columns:
            is_categorical = ("object" in c.dtype) or ("category" in c.dtype)
            if is_categorical:
                ratio = c.unique / summary.n_rows
                if c.unique >= 20 and ratio >= 0.5:
                    high_cardinality_columns[c.name] = float(ratio)
