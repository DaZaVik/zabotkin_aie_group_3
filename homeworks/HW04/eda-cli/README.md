...
- `quality-flags-from-csv` – Принимает CSV-файл и возвращает детальный набор булевых флагов качества данных без агрегированного скоринга.
---
...
```
5. `POST /quality-flags-from-csv` – дополнительный эндпоинт (расширение задания)

Эндпоинт, добавленный в рамках расширения задания S04.

Принимает CSV-файл и возвращает **полный набор булевых флагов качества**
(включая флаги из HW03), без агрегированного `quality_score` и без решения
`ok_for_model`.

Используется, когда требуется получить детальную диагностику качества данных,
а не итоговую эвристическую оценку.
